...
Q1 命中第 2 或第 3 支、或第 4 支且 diff 超過自審門檻 → **先載入 `agent-dispatch:dev-flows` skill**，照它的步驟走。裡面是 TDD 三步、UI 視覺比對四步、執行→驗證→修正循環的完整條款（含各步的 briefing 必含事項），不要憑印象執行。

## 驗證鐵律（必須遵守）

不可直接採信執行 sub agent 的結論。Q1 第 1–3 支的驗證方式寫在 `agent-dispatch:dev-flows` 各自流程內（主 agent 重跑機制／截圖定案），但機械綠 ≠ 任務完成——主 agent 仍要便宜複核 diff（改動範圍有無溢出、有沒有順手改壞別的）。
